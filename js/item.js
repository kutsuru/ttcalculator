...
,[1083,9,0,70,4,0,0,80,"Glorious Staff Of Destruction","","<span style='font-size: 14px;color:#ff0000;font-weight: bold'><br>[ONLY PvP/BG/WoE]</span><br><br><b>[Every Refine Level]</b><br>INT + 1<br>ASPD + 2%<br>Decrease Cast Time by 2%.<br>Adds 1% magical defense bypassing on Demi-human monster.<br>Adds MATK + 3% for each refine level for 6 seconds when using magic attacks at a 1% chance per refine.<br><br><b>[Refine Rate 7~10]</b><br>30% Stun Resistance.<br>",4,3,5,5,89,20,177,10,317,20,194,1,195,1,0]
,[1084,9,77,70,4,0,0,80,"Glorious Arc Wand","","<span style='font-size: 14px;color:#ff0000;font-weight: bold'>[ONLY PvP/BG/WoE]</span><br><br><b>[Every Refine Level]</b><br>INT + 1<br>Decrease cast time by 1%.<br><b>[2 Refine Levels]</b><br>Decreases after-cast delay by 1%.<br>Increase MATK against Demi-human monsters by 1%.<br>Adds a 1% chance to inflict 5% more magic damage to medium size targets for 15 seconds when casting magic.",5,5,177,15,317,20,194,1,0]
...
,[1098,16,144,100,4,0,0,80,"Glorious Huuma Shuriken",0,"<span style='font-size: 14px;color:#ff0000;font-weight: bold'><br>[ONLY PvP/BG/WoE]</span><br><br><b>[Every Refine Level]</b><br>HP + 150.<br>Increases damage with [Throw Huuma Shuriken] by 3%.<br>Adds 1% defense bypassing on Demi-human monster.<br>Adds a 5% chance to auto-cast level 2 [Critical Wounds] after using [Throw Huuma Shuriken].<br>Adds a 5% chance to auto-cast [Illusionary Shadow] at the level learned after using [Final Strike]. If not learned, level 1 is used.<br><b>[Every Refine Rate 6~10]</b><br>5% of your current MaxHP remains after using [Final Strike].",37,95,187,20,89,20,194,1,0]
...
,[1385,16,144,250,4,4,100,0,"-Hira Shurikat","","Negates the need for ninja stones<br><font color='blue'>SQI</font>",20,4,5,15,12,13,15,20,16,20,18,8,77,20,79,20,89,20,0] //custom Talon Tales SQI
   ...
   var tempScrollTop = window.pageYOffset;
	...
	if(ItemID[CBI][1] == 1){
		...
		for(i = 2; i < ItemID[CBI].length; i++){
		 itemLink += " <a class=\"linkW\" href=\"https://file5s.ratemyserver.net/items/small/"+ ItemID[CBI][i] + "/\" target=\"_blank\"><b>" + ItemID[CBI][i] + "</b></a>";
		}
         ...
         for(var j=1;w_SE[i][j] != "NULL";j++){
            ...
            for(var k=0;k<slotsExceptList.length;k++){
               if(slotsExceptList[k][1] == w_SE[i][j] && slotsExceptList[k][0] == w_SE[i][0]){
                  ...
                  SENstr += "<a class=\"linkW\" href=\"https://file5s.ratemyserver.net/items/small/"+ ItemID[slotsExceptList[k][1]][slotVersion + 2] + "/\" target=\"_blank\"><b>" + "<font color='blue'>["+ ItemOBJ[slotsExceptList[k][1]][8] +"][" + slotNum + "]</font></b></a>";
               exceptFound = 1;
               }
            }
            ...
            if(exceptFound == 0){
               if(ItemID[w_SE[i][j]][1]==1){
                  SENstr += "<a class=\"linkW\" href=\"https://file5s.ratemyserver.net/items/small/"+ ItemID[w_SE[i][j]][2] + "/\" target=\"_blank\"><b>" + "<font color='blue'>["+ ItemOBJ[w_SE[i][j]][8] +"]</font></b></a>";
                  if(ItemID[w_SE[i][j]][3]){
                     SENstr += "<a class=\"linkW\" href=\"https://file5s.ratemyserver.net/items/small/"+ ItemID[w_SE[i][j]][3] + "/\" target=\"_blank\"><b>" + "<font color='blue'><sup>[1]</sup></font></b></a>"
                  }
               }
               else{
